...
setting made the regression experiments less vulnerable to oversized models
...
continuation schedule under two budgets: an epoch-matched regime and a
same-time-cap regime. The latter is named \texttt{equal\_time} in the logs, but
it means that both methods used the same timeout cap; realized runtimes were
measured and could differ substantially. The benchmark used eight functions and
three random seeds for each method/budget combination. The main text reports
function-level loss differences and the quality/runtime trade-off. The essential
protocol constraints were:
