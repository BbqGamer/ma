...
curriculum learning remains useful as the model becomes larger or architecturally
stronger. A secondary goal was to inspect whether non-accuracy metrics, such as
macro-F1, top-5 accuracy, hierarchy-aware scores, and calibration, show the same
pattern as top-1 accuracy.

The intended roughness and sharpness probes were implemented in the training
code, but they were not active in this W\&B run batch. Therefore this appendix
reports performance, speed, and hierarchy-aware metrics, and leaves the direct
sharpness/Hessian question for the next experiment described in
Section~\ref{sec:cifar100-model-size-next-steps}.

\section{Experimental protocol}

All runs were downloaded from W\&B project
\texttt{curriculum-learning-ma/coarse-to-fine-curriculum}. The analyzed subset
used the tags \texttt{model-size} and \texttt{cifar100}. In total, 80 W\&B runs
...
\begin{itemize}
    ...
    \item CIFAR-style ResNets: \texttt{cifar\_resnet8},
          \texttt{cifar\_resnet20}, \texttt{cifar\_resnet32}, and
          \texttt{cifar\_resnet56},
...
The curriculum runs generally do not improve the area under the test-accuracy
curve. Figure~\ref{fig:cifar100-model-size-auc-gain} shows the AUC gain for the
best curriculum run of each model. All values are negative.
...
accuracy gains, the alternative metrics are also weak or negative.
...
drawn from this W\&B batch, because roughness probes were disabled:

\begin{verbatim}
config_roughness_probes = False
\end{verbatim}

Therefore the exported histories do not contain:
...
This is an important negative finding about the current data, not about the
method itself. The sharpness question remains open and should be tested with the
focused follow-up in Section~\ref{sec:cifar100-model-size-next-steps}.
...
\section{Next experiment plan}
\label{sec:cifar100-model-size-next-steps}

The next experiment should answer the sharpness and smoothness question without
rerunning the full grid. I propose a focused roughness subset:

\begin{itemize}
    \item \texttt{cnn\_w1}: baseline and \texttt{curr10},
    \item \texttt{cnn\_w4}: baseline and \texttt{curr5},
    \item \texttt{cifar\_resnet8}: baseline and \texttt{curr20},
    \item \texttt{cifar\_resnet32}: baseline and \texttt{curr10},
    \item \texttt{resnet18}: baseline and \texttt{curr20}.
\end{itemize}

The recommended roughness settings are:
...
These runs should test whether the models that benefit from curriculum also show
lower sharpness, lower top Hessian eigenvalue, lower Hessian Frobenius estimate,
or lower gradient noise. If the smoothing metrics improve only for models where
curriculum improves accuracy, this would provide a mechanistic explanation for
the capacity-dependent curriculum effect.

After the roughness subset, the most important robustness step is to repeat the
core CIFAR-100 size sweep with additional seeds, at least 43 and 44, for:

\begin{itemize}
    \item \texttt{cnn\_w0.5}, \texttt{cnn\_w1}, \texttt{cnn\_w2}, \texttt{cnn\_w4},
    \item \texttt{cifar\_resnet8}, \texttt{cifar\_resnet32}, \texttt{cifar\_resnet56},
    \item \texttt{resnet18}.
\end{itemize}

Only after these CIFAR-100 conclusions are stable should the same Figure-11-style
sweep be generalized to other datasets such as CIFAR-10, Fashion-MNIST, SVHN,
