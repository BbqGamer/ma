...
The main remaining gaps are more targeted. The full model-size ladder was not
repeated across many seeds, and the reported peak test accuracy should ideally
be complemented by test accuracy at the best validation epoch. The hierarchy
itself also deserves a separate ablation: learned hierarchies should be compared
against official semantic hierarchies where available and against random
hierarchies with matched cluster sizes. That would separate the value of a
meaningful grouping from the simpler effect of reducing the number of labels in
the early stage.
