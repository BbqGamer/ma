...
The run was tracked in Weights \& Biases \cite{biewald2020experiment} under ID
\texttt{yg05hkl9}. The hierarchy export was enabled and the training log confirms
that a teacher-embedding hierarchy was written to:

\begin{verbatim}
/runpod-volume/teachers/teacher-cifar100-resnet18-imagenet30-hierarchy
\end{verbatim}

The hierarchy was exported from the validation split embeddings after the run
finished. The run log confirms the export path and the resulting files were
later recovered from the Runpod object store:
\texttt{hierarchy.json}, \texttt{hierarchy.md}, and
\texttt{distance\_matrix\_teacher\_embeddings.npy}. This makes it possible to
analyze the teacher hierarchy directly rather than treating the run only as a
...
Appendix~\ref{app:cifar100-model-size-curriculum}. The comparison is not
strictly apples-to-apples, because the protocols differ: the earlier appendix
used an uninitialized ResNet-18 with Adam, no scheduler, and a 100-epoch budget,
while the present run used ImageNet initialization, SGD, augmentation, and only
30 epochs. Even with that caveat, the magnitude of the improvement is large
...
curriculum gains were limited or unstable. The teacher-derived hierarchy still
contains imperfections---for example, \texttt{lamp, mushroom, rocket, snail} is
an awkward group, and some broad mammal clusters remain heterogeneous---but its
overall organization is markedly more plausible. That makes it a credible
candidate for testing whether hierarchy quality, rather than output-space
curriculum as such, was the limiting factor in the earlier experiments.
...
The next version of this appendix should add image-grid visualizations analogous
to Figures~\ref{fig:hierarchy-examples-fashionmnist},
\ref{fig:hierarchy-examples-cifar10}, and
\ref{fig:hierarchy-examples-cifar100}, using representative examples from the
teacher-derived clusters. The recovered hierarchy files are now available for
that step.
...
random and anti controls are not failures.
...
matching baseline. The teacher row uses the corrected 77.65\% transfer teacher.}
