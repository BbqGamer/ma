...
in teacher-ranked instance curricula such as Weinshall et al.\ \cite{weinshall2018curriculum}.
One plausible explanation was that the
...
condition and the same marginalized coarse-label loss. The only change is the
...
accuracy values. The random mean exceeds the baseline for all three seeds, and
all nine individual random hierarchy runs were also above their matching baseline.
...
Finally, the teacher hierarchy did not decisively outperform the self-derived
hierarchy in peak accuracy. The corrected teacher reached a much stronger operating point
...
trained cheaply and produces a plausible hierarchy. That hierarchy clearly
outperforms random grouping, and meaningful structure improves over arbitrary coarsening.
