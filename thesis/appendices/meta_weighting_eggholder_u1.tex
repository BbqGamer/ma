...
\begin{tabular}{lrrr}
\toprule
Losses & Mean improvement vs baseline & Median improvement vs baseline & Win rate vs baseline \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Paired comparison against the baseline \texttt{num\_losses=1}. Positive improvement would mean that adaptive weighting beats the baseline; in this experiment all means remain negative.}
...
conditions to beat the single-loss baseline on average.
...
\begin{figure}[h]
    ...
    \caption{Win rate of adaptive weighting versus the baseline \texttt{num\_losses=1}. A value above 0.5 would indicate an advantage over the baseline. None of the adaptive settings exceed this threshold.}
    \label{fig:meta-eggholder-u1-winrate}
\end{figure}
...
The single hard-target baseline remains difficult to beat. Although adaptive
