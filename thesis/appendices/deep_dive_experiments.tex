...
\begin{tabular}{llrrl}
\toprule
Function & Regime & Quality delta & Runtime ratio & Lower final loss \\
\midrule
...
\begin{figure}[h]
    ...
    \caption{Function-level hard-validation loss delta by function and budget
    regime, using the same direct-minus-curriculum statistic as the main
    regression table. Positive values favor curriculum.}
...
\begin{figure}[h]
    ...
    \caption{CIFAR-100 curriculum-length sensitivity. Values are best test-accuracy
    gains in percentage points relative to the matching baseline.}
...
Negative trajectory-AUC gains are common, reinforcing the interpretation that
curricula can improve the best solution reached while still delaying the
fine-label trajectory.
...
control. The learned hierarchy outperforms the random mean for every seed and
also exceeds the best random hierarchy tried for each seed. This is stronger than a
