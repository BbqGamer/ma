...
final model quality or optimization behavior relative to a strong non-curriculum
...
The main baseline is direct training on the final target objective or final label
space. Curriculum variants are compared against this baseline using matched
architectures, training budgets, and, where possible, hyperparameter tuning. The
experiments also include ablations such as random label hierarchies, because an
observed gain from a curriculum may come from reduced early-task complexity
rather than from meaningful task structure.
...
quality measure is accuracy. Additional diagnostics, including roughness and
sharpness probes, are used only as suggestive evidence about optimization
mechanisms, not as causal proof.

\section{Reproducibility}

The implementation uses scripted experiment entry points, Optuna sweeps for
hyperparameter search, and MLflow tracking for parameters, metrics, and
artifacts. Detailed run reports are kept in the appendices so that the main text
can focus on the research claims while preserving the connection to the actual
experiments.
...
multi-loss schedules make difficult target functions easier to learn.

\section{Experimental setup}

The regression experiments use benchmark functions with non-convex structure.
For each function, the model is trained either directly on the final target or
through a sequence of smoothed targets. The curriculum variants include fixed
schedules and dynamic multi-loss weighting. Hyperparameter sweeps are used to
avoid comparing curriculum methods against under-tuned single-target baselines.
...
The experiments use output-space curricula inspired by label hierarchies. During
early training, the model predicts coarse labels; later stages introduce finer
labels until the final classification task is reached. The track includes fixed
schedules, an adaptive plateau schedule, model-capacity comparisons, cross-dataset
checks, and a random-hierarchy ablation.
