...
## 2. Potential Future Application
...
*   **Option A: Physics-Informed Neural Networks (PINNs) / Fluid Dynamics**
    ...
    *   *The Benefit:* The PINN baseline is standard. The literature explicitly staVtes that standard PINNs fail at low viscosity and *require* a curriculum. My meta-learning algorithm perfectly solves this known gap.
...
2.  Baseline Comparisons:** Are there any specific alternate pacing/curriculum methods you want included in the final experiments?
3. Dataset - how many targets should I generate, for higher sizes it breaks

# After the meeting
We agreed that we should obtain strong baselines and freeze parameters when comparing if curriculum improves the training. The optuna parameter tuning was useful then.
We are not pivoting, we will be still performing regression on mathematical functions, for now Ackley and Eggholder. We should think about optimization budget in all the
experiments to show that our method requires less steps (or less wall clock time) to reach the same performance as the baselines. It seems that we shoult not take
only the best runs from optuna because curriculum might be less useful when the hyperparameters are well tuned, but it might be more useful when the hyperparameters are not well tuned.

Tasks for next week:
- [ ] refactor the code to allow running the same experiment with some specific parameters frozen (e.g. learning rate, model capacity) but with different curriculum methods. 
- [ ] run the curriculum learning experiments with the same parameters as the baselines, and compare the results.
